...
### F5. AI 귀가 코칭
- **동작:** 경로 비교 결과를 OpenAI API(gpt-4o-mini)에 전달하여 2-3문장의 자연어 귀가 조언을 생성
- **수용 기준**
  - [ ] 비교 카드 하단에 "AI 귀가 코칭 받기" 버튼 노출
  - [ ] 클릭 시 로딩 표시 후 자연어 코칭 말풍선 표시
  - [ ] 출발지/도착지, 소요시간, 리스크, 실시간 ETA 정보를 프롬프트에 포함
  - [ ] API 키 미설정 시 폴백 메시지 반환 (서비스 중단 없음)
  - [ ] 에러 시 재시도 버튼 제공
